...
class ElemtE07(object):
    ...
    def __init__(self,x,y=None,p=None):
        ...
        if isinstance(x, ElementDeZnZ) and isinstance(y, ElementDeZnZ):
            assert x.n == y.n, "Les modulos des ElementDeZnZ ne sont pas les memes"
            assert isprime(x.n)==True, "p doit etre premier"
            assert ElementDeZnZ(y**2,x.n) == ElementDeZnZ(x**3 + 7, x.n), "La propriété n'est pas respecté"
            self.x = x
            self.y = y
            self.p = p
            ...
            assert isprime(p)==True, "p doit etre premier"
            self.x = ElementDeZnZ(x,p)
            self.y = "INF"
            self.p = p
        else:
            assert isprime(p)==True, "p doit etre premier"
            assert ElementDeZnZ(y**2,p) == ElementDeZnZ(x**3 + 7,p), "La propriété n'est pas respecté"
            self.x = ElementDeZnZ(x,p)
    ...
    def lDesElements(p=47):
        ...
        True
        """
        lde = [ElemtE07(0,"INF",5)]
        for x in range(p):
            for y in range(p):
                if(ElementDeZnZ(y**2, p) == ElementDeZnZ(x**3 +7, p)):
                    lde.append(ElemtE07(x,y,p))
        return lde
    ...
    def __add__(self,other):
        ...
        ElemtE07(3,9,47)
        """
        assert isinstance(other, ElemtE07), "other doit etre un ElemtE07"
        assert other.x - self.x, "division par 0 impossible"
        if(isinstance(self.x, ElementDeZnZ) and isinstance(self.y, ElementDeZnZ)):
            lambda_ = (other.y-self.y)/(other.x-self.x)
            x=lambda_**2 - self.x - other.x
            y=-1*(lambda_*(x-self.x)+self.y)
            return ElemtE07(ElementDeZnZ(x, self.p),ElementDeZnZ(y, self.p))
        elif(other.y =="INF"):
            return self
        elif(self.y =="INF"):
            return other
        elif(self.y =="INF" and other.y =="INF")
